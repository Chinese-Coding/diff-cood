...
class LiftSplatShoot(nn.Module):
    def __init__(self, lss_args: DictConfig, device: torch.device):
        super(LiftSplatShoot, self).__init__()
        self.grid_conf = lss_args["grid_conf"]  # 网格配置参数
        self.data_aug_conf = lss_args["data_aug_conf"]  # 数据增强配置参数
        # 划分网格
        dx, bx, nx = gen_dx_bx(self.grid_conf["xbound"], self.grid_conf["ybound"], self.grid_conf["zbound"])

        self.dx = dx.clone().detach().requires_grad_(False).to(device)  # [0.4,0.4,20]
        self.bx = bx.clone().detach().requires_grad_(False).to(device)  # [-49.8,-49.8,0]
        self.nx = nx.clone().detach().requires_grad_(False).to(device)  # [250,250,1]

        self.depth_supervision = lss_args["depth_supervision"]
        self.downsample = lss_args["img_downsample"]  # 下采样倍数
        self.camC = lss_args["img_features"]  # 图像特征维度
        self.frustum = self.create_frustum().clone().detach().requires_grad_(False).to(device)  # frustum: DxfHxfWx3(41x8x16x3)
        self.use_quickcumsum = True
        self.D, _, _, _ = self.frustum.shape  # D: 41
        self.camera_encoder_type = lss_args["camera_encoder"]
    ...
    def forward(self, x, rots, trans, intrins, post_rots, post_trans):
        ...
        # post_rots: [4,4,3,3]
        # post_trans: [4,4,3]
        # TODO: 为什么 HEAL 使用的 Lift Splat Shoot 方法这么复杂, 相较于 Lift Splat Shoot 这篇论文最原始的方法怎么有这么多的修改
        #       还有, 为什么 HEAL 没有使用 `bevencode` 而是直接将 `x` 输出? 然后将 x 送入 backbone 进行处理?
