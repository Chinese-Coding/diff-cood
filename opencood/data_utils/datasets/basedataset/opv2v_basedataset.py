...
class OPV2VBaseDataset(Dataset):
    ...
    def reinitialize(self):
        ...
        for i, scenario_folder in enumerate(self.scenario_folders):
            ...
            for j, cav_id in enumerate(cav_list):
                if j > self.max_cav - 1:
                    logger.warning(f"too many cavs reinitialize in {scenario_folder}")
                    break
                ...
                yaml_files = [x for x in yaml_files if not ("2021_09_09_13_20_58" in x)]  # 过滤一下不然会报错
    ...
    def generate_object_center_lidar(self, cav_contents, reference_lidar_pose):
        ...
        """
        return self.post_processor.generate_object_center_lidar(cav_contents, reference_lidar_pose)
