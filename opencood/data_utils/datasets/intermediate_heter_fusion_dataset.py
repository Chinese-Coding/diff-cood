...
def getIntermediateheterFusionDataset(cls):
    ...
    class IntermediateheterFusionDataset(cls):
        def __init__(self, params, visualize, train=True):
            ...
            self.anchor_box = self.post_processor.generate_anchor_boxes()
            self.anchor_box_torch = torch.from_numpy(self.anchor_box)
            logger.info(f"anchor box shape: {self.anchor_box_torch.shape}")
            ...
            for modality_name, modal_setting in params["heter"]["modality_setting"].items():
                ...
                if modal_setting["sensor_type"] == "lidar":
                    ...
                    setattr(
                        self,
                        f"data_aug_conf_{modality_name}",
                        modal_setting["data_aug_conf"],
                    )
        ...
        def get_item_single_car(self, selected_cav_base, ego_cav_base):
            ...
            label_dict = self.post_processor.generate_label(object_bbx_center, self.anchor_box, object_bbx_mask)
        ...
        def __getitem__(self, idx):
            ...
            label_dict = self.post_processor.generate_label(object_bbx_center, self.anchor_box, mask)
        ...
        def post_process(self, data_dict, output_dict):
            ...
            gt_box_tensor = self.post_processor.generate_gt_bbx_heal(data_dict)
