...
def main(args):
    ...
    train_dataset = build_dataset(args, visualize=False, train=True)
    ...
    for epoch in range(first_epoch, args.train_epoches):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            total_loss = loss_fn(
                ...
                label["pos_equal_one"].to(device), label["neg_equal_one"].to(device), label["targets"].to(device)
            )
            # fmt: on
            ...
            if step % args.save_vis_interval == 0:
                ...
                ego["transformation_matrix_clean"] = torch.from_numpy(np.identity(4)).float()
                ego["transformation_matrix"] = torch.from_numpy(np.identity(4)).float()
                pred_box_tensor, pred_score, gt_box_tensor = train_dataset.post_process(batch, output_dict)
...
if __name__ == "__main__":
    ...
    args.output_dir = "~/Desktop/logs/pcd_detection_2025_03_01"
    args.batch_size = 2
    args.train_epoches = 20
    main(args)
