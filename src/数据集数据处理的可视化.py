...
def main(args):
    ...
    # 随机选择 3 个 anchor
    num_samples = 300
    random_indices = np.random.choice(anchor_boxes.shape[0], num_samples, replace=False)  # 不重复选择
    random_anchors = anchor_boxes[random_indices]
    infer_result = {"gt_box_tensor": batch["gt_bbx_list"][0], "pred_box_tensor": torch.tensor(random_anchors)}
