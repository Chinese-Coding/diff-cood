...
from data_related.transform_funs import img_transform, pcd_transform, dep_transform
...
def expand_paths(args):
    for key, value in args.items():
        if isinstance(value, str):
            args[key] = os.path.expanduser(value)  # 扩展路径
        elif isinstance(value, dict):
            expand_paths(value)  # 递归处理嵌套的字典


def main(args):
    # 路径展开
    args.output_dir = os.path.expanduser(args.output_dir)
    expand_paths(args.pretrained_model_args)
    ...
    # 加载 dep 的处理函数
    infer_dataset.set_transform(
        img_transform(), pcd_transform(args.cav_lidar_range, args.ratio), dep_transform(args.cav_lidar_range, args.ratio)
    )

    """特征提取网络"""
    prepare_processor = PrepareProcessor(args.pretrained_model_args.diffusion, args.pretrained_model_args.revision)
    pcd_unet = DetectionUNet2DConditionModel.from_pretrained(
        args.pretrained_model_args.diffusion, revision=args.pretrained_model_args.revision, subfolder="unet"
    )
    controlnet = ControlNetModel.from_pretrained(
        args.pretrained_model_args.controlnet, revision=args.pretrained_model_args.revision
    )
    """显存优化部分"""
    ...
    for step, batch in enumerate(infer_dataloader):
        ...
        with torch.no_grad():
            ...
            down_block_res_samples, mid_block_res_sample = controlnet(
                ...
                controlnet_cond=batch["dep"].to(device, dtype=weight_dtype),
                return_dict=False,
            )
            model_pred = pcd_unet(
                noisy_latents,
...
if __name__ == "__main__":
    ...
    args = OmegaConf.load(os.path.expanduser("~/fleet/diff-cood/train_detection_with_dep.yaml"))
    args.resume_file_det = "~/Desktop/logs/pcd_detection_2025_02_27_afternoon/checkpoint-8-det.pth"
    args.output_dir = "~/Desktop/logs/pcd_detection_2025_02_27_afternoon_infer"
