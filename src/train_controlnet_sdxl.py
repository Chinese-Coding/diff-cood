...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Path to an improved VAE to stabilize training. For more details check out:"
...
def get_train_dataset(args, accelerator):
    ...
    if args.conditioning_image_column is None:
        ...
        if conditioning_image_column not in column_names:
            raise ValueError(
                f"`--conditioning_image_column` value '{args.conditioning_image_column}' not found in dataset columns. Dataset"
                f" columns are: {', '.join(column_names)}"
            )
